...
export async function sendResearchSubmissionAlert(submission: ResearchSubmissionRecord) {
  ...
  const followUpEmail = asString(response.followUpEmail) ?? submission.userEmail ?? null;
  ...
  await resend.emails.send({
    ...
    text: [
      ...
      `Follow-up email: ${followUpEmail ?? "Not provided"}`,
    ...
    html: `
      <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #1f2937;">
        ...
        <p><strong>Follow-up email:</strong> ${followUpEmail ?? "Not provided"}</p>
